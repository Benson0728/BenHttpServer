...
public class DeleteRouteHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
        @Override
        protected void channelRead0 (ChannelHandlerContext channelHandlerContext, FullHttpRequest fullHttpRequest) throws
        Exception {
            System.out.println("进入DeleteRouteHandler");
            //预设返回状态码为404
            ...
            if (fullHttpRequest.method() == HttpMethod.DELETE) { //如果是Delete请求则处理
                ...
                //扫描所有Controller包下带有@Delete的方法
                MultiValueMap<Class<?>, Method> map = RestfulScanner.scan(Delete.class);
                ...
                for (Map.Entry<Class<?>, List<Method>> entry : map.entrySet()) {
                    ...
                    for (Method method : methods) {
                        ...
                        if (method.getAnnotation(Delete.class).value().equals(uri)) { //不含PathVariable
                                try {
                                ...
                                break outerLoop;
                        } else if (pattern.matcher(method.getAnnotation(Delete.class).value()).find()) {
                            //将注释路径和请求路径按"/"分割
                            String[] split1 = method.getAnnotation(Delete.class).value().split("/");
                            String[] split2 = uri.split("/");
                            //路径层数不一样直接跳出
                            if (split1.length != split2.length) continue;
                            int pathVariableNums = PathVariableUtils.getPathVariableNums(method.getAnnotation(Delete.class).value());
                            //路径参数个数不一样直接跳出
                            ...
                            List<String> pathVariableName = PathVariableUtils.getPathVariables(method.getAnnotation(Delete.class).value());
         ...
         * 不含PathVariable
         */
        private static Object handleFormData (ByteBuf content, Map.Entry < Class < ?>,List<Method>>entry, Method
        method, HttpHeaders headers) throws
         ...
         */
        private static Object handleFormData (ByteBuf content, Map.Entry < Class < ?>,List<Method>>entry, Method
        method, Map < String, String > pathVariables, HttpHeaders headers) throws
        NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
            Parameter[] parameters = method.getParameters();
            String contentString = content.toString(StandardCharsets.UTF_8);
            Map<String, FormDataItem> formData = parseFormData(contentString);
