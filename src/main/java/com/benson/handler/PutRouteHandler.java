...
public class PutRouteHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    @Override
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, FullHttpRequest fullHttpRequest) throws Exception {
        System.out.println("进入PutRouteHandler");
        //预设返回状态码为404
        ...
        if (fullHttpRequest.method() == HttpMethod.PUT) { //如果是Put请求则处理
            ...
            MultiValueMap<Class<?>, Method> map = RestfulScanner.scan(Put.class);
            ...
            for (Map.Entry<Class<?>, List<Method>> entry : map.entrySet()) {
                ...
                for (Method method : methods) {
                    ...
                    if (method.getAnnotation(Put.class).value().equals(uri)) { //不含PathVariable
                    ...
                    } else if (pattern.matcher(method.getAnnotation(Put.class).value()).find()) {
                        //将注释路径和请求路径按"/"分割
                        String[] split1 = method.getAnnotation(Put.class).value().split("/");
                        String[] split2 = uri.split("/");
                        //路径层数不一样直接跳出
                        if (split1.length != split2.length) continue;
                        int pathVariableNums = PathVariableUtils.getPathVariableNums(method.getAnnotation(Put.class).value());
                        //路径参数个数不一样直接跳出
                        ...
                        List<String> pathVariableName = PathVariableUtils.getPathVariables(method.getAnnotation(Put.class).value());
